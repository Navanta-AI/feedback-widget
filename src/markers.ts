...
export class MarkerManager {
  ...
  /**
   * Resolve the feedback's first annotation to its live anchor element and
   * scroll it into view. Uses `Element.scrollIntoView`, which walks up to the
   * nearest scroll container — so it works whether the host scrolls the window
   * OR a nested `overflow:auto` element (e.g. a portal `<main>`). The old
   * `window.scrollTo(scrollX, scrollY)` path silently did nothing on hosts
   * whose window never scrolls. Returns true when an anchor resolved.
   */
  scrollToFeedback(feedback: FeedbackResponse): boolean {
    for (const annotation of feedback.annotations) {
      const resolved = resolveAnnotation(toAnchorData(annotation), toRectData(annotation));
      if (resolved) {
        resolved.element.scrollIntoView({ behavior: "smooth", block: "center", inline: "nearest" });
        return true;
      }
    }
    return false;

...
import { ExportButton, ImportButton, parseFeedbacksJson } from "./export-utils.js";
...
export class Panel {
  ...
  /** Status filter — lives inside the combined filter dropdown (no segmented control). */
  private statusFilter: "all" | FeedbackStatus = "all";
  private statusOptions!: ReadonlyArray<{ value: "all" | FeedbackStatus; label: string; icon: string; color: string; bg: string }>;
  ...
  private readonly importBtn: ImportButton;
  ...
  constructor(
    ...
    // Icon-only action — the label lives in aria-label + title (tooltip).
    ...
    this.deleteAllBtn.title = this.t("panel.deleteAll");
    ...
    // Import button — upload a previously exported JSON to recreate its pins.
    this.importBtn = new ImportButton((raw) => this.importFromJson(raw), this.t);

    const headerRight = el("div", { class: "sp-panel-header-right" });
    headerRight.appendChild(this.importBtn.element);
    ...
    // Filter bar — a single combined dropdown (status + type sections). The
    // page scope ("This page" / "All pages") renders as an underline tab strip
    ...
    filters.appendChild(this.buildScopeSegmented());
    ...
    this.detail = new DetailView(
      colors,
      {
        ...
        onGoToAnnotation: (fb) => {
          if (fb.annotations.length === 0) return;
          // Prefer scrolling the live anchor element into view (handles nested
          // scroll containers). Fall back to the saved window scroll offset for
          // hosts where the window is the scroller and the anchor can't resolve.
          const scrolled = this.markers.scrollToFeedback(fb);
          if (!scrolled) {
            const ann = fb.annotations[0];
            if (ann) window.scrollTo({ left: ann.scrollX, top: ann.scrollY, behavior: "smooth" });
          }
  ...
  /** Build the combined filter dropdown — status + type in one control. */
  ...
  private renderTypeDropdownTrigger(): void {
    const activeType = this.typeOptions.find((o) => this.activeFilters.has(o.value)) ?? this.typeOptions[0];
    const activeStatus = this.statusOptions.find((o) => o.value === this.statusFilter) ?? this.statusOptions[0];
    if (!activeType || !activeStatus) return;

    // Trigger summary: default reads "Filter: All"; with picks it lists the
    // non-default selections ("Open", "Open · Bug", …). Chip tint follows the
    // status pick first (the primary workflow), else the type pick.
    const statusFiltered = activeStatus.value !== "all";
    const typeFiltered = activeType.value !== "all";
    const filtered = statusFiltered || typeFiltered;
    const lead = statusFiltered ? activeStatus : typeFiltered ? activeType : activeStatus;
    const parts = [...(statusFiltered ? [activeStatus.label] : []), ...(typeFiltered ? [activeType.label] : [])];
    const valueText = filtered ? parts.join(" · ") : this.t("panel.filterAll");

    this.typeDropdownBtn.replaceChildren();
    this.typeDropdownBtn.style.setProperty("--sp-chip-color", filtered ? lead.color : this.colors.accent);
    this.typeDropdownBtn.style.setProperty("--sp-chip-bg", filtered ? lead.bg : this.colors.accentLight);
    this.typeDropdownBtn.dataset.filter = `${activeStatus.value}/${activeType.value}`;
    this.typeDropdownBtn.classList.toggle("sp-filter-dropdown-btn--filtered", filtered);
    this.typeDropdownBtn.setAttribute("aria-label", `${this.t("filter.label")}: ${valueText}`);

    const iconWrap = el("span", { class: "sp-filter-dropdown-btn__icon" });
    iconWrap.appendChild(parseSvg(filtered ? lead.icon : ICON_LAYERS));
    ...
    setText(prefix, this.t("filter.label"));
  ...
  /** Render one selectable row of the combined filter menu. */
  private buildFilterOption(
    option: { value: string; label: string; icon: string; color: string; bg: string },
    isActive: boolean,
    onSelect: () => void,
  ): HTMLButtonElement {
  ...
  private openTypeDropdown(): void {
    ...
    this.typeDropdownMenu.setAttribute("aria-label", this.t("filter.label"));
    this.typeDropdownBtn.setAttribute("aria-expanded", "true");

    // Section: status
    const statusHeader = el("div", { class: "sp-filter-dropdown-section" });
    setText(statusHeader, this.t("status.label"));
    this.typeDropdownMenu.appendChild(statusHeader);
    for (const option of this.statusOptions) {
      this.typeDropdownMenu.appendChild(
        this.buildFilterOption(option, this.statusFilter === option.value, () => this.selectStatusFilter(option.value)),
      );
    }

    // Section: type
    const typeHeader = el("div", { class: "sp-filter-dropdown-section" });
    setText(typeHeader, this.t("type.label"));
    this.typeDropdownMenu.appendChild(typeHeader);
    for (const option of this.typeOptions) {
      this.typeDropdownMenu.appendChild(
        this.buildFilterOption(option, this.activeFilters.has(option.value), () => this.selectTypeFilter(option.value)),
      );
    }
    ...
    requestAnimationFrame(() => {
      this.typeDropdownOutsideHandler = (e: MouseEvent) => {
        // Use composedPath() — the listener is on `document`, so shadow-DOM
        // retargeting would rewrite e.target to the host, making a plain
        // `contains(e.target)` check always false for in-shadow clicks (which
        // would close-then-reopen on a trigger click). composedPath() keeps the
        // real in-shadow elements, so a click on the trigger is correctly seen
        // as "inside" and left to the trigger's own toggle handler.
  ...
  /**
   * Import pins from a previously exported JSON file.
   *
   * Each entry is re-submitted through the regular client path scoped to the
   * CURRENT project, keeping the original `clientId` so idempotent stores
   * merge re-imports instead of duplicating. Resolved entries are re-resolved
   * after creation. Per-item failures are reported via `feedback:error` but
   * don't abort the batch; the whole call throws only when nothing imported.
   *
   * @returns the number of feedbacks successfully imported.
   */
  private async importFromJson(raw: string): Promise<number> {
    const pins = parseFeedbacksJson(raw); // throws on unusable files → error flash
    const imported: FeedbackResponse[] = [];
    let lastError: unknown = null;

    for (const pin of pins) {
      try {
        let response = await this.client.sendFeedback({ ...pin.payload, projectName: this.projectName });
        if (pin.resolved && response.status !== "resolved") {
          response = await this.client.resolveFeedback(response.id, true);
        }
    ...
    if (imported.length === 0) {
      throw lastError instanceof Error ? lastError : new Error("Import failed: no feedbacks could be created");
    }

    // Drop markers for imported pins that belong to the current page (same
    // scope rule as the regular submit flow), then reload the list + stats.
    const scope = this.getScope();
    for (const fb of imported) {
      if (!this.scopeAnnotationsByUrl || fb.url === scope.url) {
        this.markers.addFeedback(fb, this.markers.count + 1);
      }
    }
    this.bus.emit("feedback:imported", imported);
    await this.loadFeedbacks();

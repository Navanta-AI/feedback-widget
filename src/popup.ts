...
// The popup mounts in the light DOM (document.body), not the widget's shadow
// root, so it can't read the shadow-scoped --sp-font. It CAN read --font-geist-sans
// from the host page's :root, so reference it directly and fall back to Geist/Inter.
const POPUP_FONT = 'var(--font-geist-sans, "Geist"), "Inter", system-ui, -apple-system, sans-serif';
...
export class Popup {
  ...
  constructor(
    ...
    for (const option of typeOptions) {
      ...
      btn.style.cssText = `
        ...
        font-family:${POPUP_FONT};
        font-size:13px;font-weight:500;color:${this.colors.textSecondary};
    ...
    this.textarea.style.cssText = `
      ...
      color:${this.colors.text};font-family:${POPUP_FONT};
    ...
    this.textarea.addEventListener("focus", () => {
      if (this.submittingState) return;
      this.textarea.style.borderColor = this.colors.text;
      this.textarea.style.boxShadow = `0 0 0 3px ${this.colors.ring}80`;
    ...
    this.cancelBtn.style.cssText = `
      ...
      border:1px solid ${this.colors.border};
      background:${this.colors.glassBg};
      color:${this.colors.textSecondary};font-family:${POPUP_FONT};
    ...
    this.cancelBtn.addEventListener("mouseenter", () => {
      if (this.submittingState) return;
      this.cancelBtn.style.background = this.colors.bgHover;
      this.cancelBtn.style.color = this.colors.text;
    });
    this.cancelBtn.addEventListener("mouseleave", () => {
      if (this.submittingState) return;
      this.cancelBtn.style.background = this.colors.glassBg;
      this.cancelBtn.style.color = this.colors.textSecondary;
    });

    this.submitBtn = document.createElement("button");
    this.submitBtn.style.cssText = `
      height:28px;padding:0 14px;border-radius:8px;
      border:none;background:${this.colors.primary};
      color:${this.colors.primaryFg};font-family:${POPUP_FONT};
    ...
    this.submitBtn.addEventListener("mouseenter", () => {
      if (this.submittingState || this.submitBtn.disabled) return;
      this.submitBtn.style.background = this.colors.primaryHover;
    });
    this.submitBtn.addEventListener("mouseleave", () => {
      if (this.submittingState) return;
      this.submitBtn.style.background = this.colors.primary;
    });
  ...
  private selectType(type: FeedbackType, container: HTMLElement): void {
    ...
    for (const btn of buttons) {
      ...
      btn.style.color = isActive ? color : this.colors.textSecondary;
  ...
  private resetTypeButtons(): void {
    ...
    for (const btn of buttons) {
      ...
      btn.style.background = this.colors.glassBg;
      btn.style.borderColor = this.colors.border;
      btn.style.color = this.colors.textSecondary;
